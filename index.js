...
let scoreRed = 12;
let scoreBlack = 12;
let numberPionRed = document.querySelectorAll(".Redpion");
let numberPionBlack = document.querySelectorAll(".BlackPion");
...
let playTable = [
  ...
  null,
  18,
  null,19,null,null,20,null,21,null,22,null,23,
];
...
function getMove() {
  if (playTable[toggledPion.toggledPionTableId + 7] === null &&
    allCases[toggledPion.toggledPionTableId + 7].classList.contains(
      "noPieceHere"
    ) !== true
  ) {
  ...
  if (
    ...
    allCases[toggledPion.toggledPionTableId + 9].classList.contains(
      "noPieceHere"
    ) !== true
  ) {
...
function checkJumpMove() {
  if (playerTurn) {
    if (
        playTable[toggledPion.toggledPionTableId + 14] === null &&
      allCases[toggledPion.toggledPionTableId + 14].classList.contains(
        "noPieceHere"
      ) !== true &&
    ...
    if (
        playTable[toggledPion.toggledPionTableId + 18] === null &&
      allCases[toggledPion.toggledPionTableId + 18].classList.contains(
        "noPieceHere"
      ) !== true &&
...
function makeMove(number) {
  ...
  if (playerTurn) {
    if (toggledPion.isKing) {
      allCases[
        toggledPion.toggledPionTableId + number
      ].innerHTML = `<p class="red-piece king" id="${toggledPion.pieceId}"></p>`;
      numberPionRed = document.querySelectorAll("p");
    } else {
      allCases[
        toggledPion.toggledPionTableId + number
      ].innerHTML = `<p class="red-piece" id="${toggledPion.pieceId}"></p>`;
      numberPionRed = document.querySelectorAll("p");
    }
  } else {
    if (toggledPion.isKing) {
      allCases[
        toggledPion.toggledPionTableId + number
      ].innerHTML = `<p2 class="black-piece king" id="${toggledPion.pieceId}"></p2>`;
      numberPionBlack = document.querySelectorAll("p2");
    } else {
      allCases[
        toggledPion.toggledPionTableId + number
      ].innerHTML = `<p2 class="black-piece" id="${toggledPion.pieceId}"></p2>`;
      numberPionBlack = document.querySelectorAll("p2");
    }
  }
...
function changeData(indexOfBoardPiece, modifiedIndex, removePiece) {
    playTable[indexOfBoardPiece] = null;
    playTable[modifiedIndex] = parseInt(toggledPion.pieceId);
  if (playerTurn && toggledPion.pieceId < 12 && modifiedIndex >= 57) {
    document.getElementById(toggledPion.pieceId).classList.add("king");
  }
  if (playerTurn === false && toggledPion.pieceId >= 12 && modifiedIndex <= 7) {
    document.getElementById(toggledPion.pieceId).classList.add("king");
  }
  if (removePiece) {
    playTable[removePiece] = null;
    if (playerTurn && toggledPion.pieceId < 12) {
      cells[removePiece].innerHTML = "";
      scoreBlack--;
    }
    if (playerTurn === false && toggledPion.pieceId >= 12) {
      cells[removePiece].innerHTML = "";
      scoreRed--;
    }
  }
